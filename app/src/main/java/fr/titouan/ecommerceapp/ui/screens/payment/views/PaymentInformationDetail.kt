...
fun PaymentInformationDetail(
    ...
    var userNameState = remember { mutableStateOf(user.value?.name ?: "") }
    var userAddressState = remember { mutableStateOf(user.value?.safePostCode ?: "") }
    var userCityState = remember { mutableStateOf(user.value?.safeCity ?: "") }

    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(16.dp)
    ) {
        PaymentStateCard(PaymentProcessID.PAYMENT)
        Row(
            ...
            Text(text = stringResource(id = R.string.payment_radio_button_payment_mode))
        }
